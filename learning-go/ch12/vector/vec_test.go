...
func TestVectorMag(t *testing.T) {
	...
	for _, c := range cases {
		mag := c.vec.Mag()
		if mag != c.expected {
			t.Errorf("Magnitude failed, execpted %v, got %v", c.expected, mag) // call Logf() and Fail()
		}
	}
}
...
func TestVectorDotProd(t *testing.T) {
	...
	expect := v1[0]*v2[0] + v1[1]*v2[1] // expect := v1[0]*v2[0] + v1[1]*v2[1]
	if actual != expect {
		t.Logf("DotPoduct2 failed, expecting %v, got %v", expect, actual)
		t.Fail() // must be, otherwise there wont be failed test
	}
...
func TestVectorAngle(t *testing.T) {
	if os.Getenv("RUN_ANGLE") == "" {
		t.Skipf("Env variable RUN_ANGLE not set, skipping:") // calls SkipNow() and Logf()
	}
	v1 := New(3.183, -7.627)
	v2 := New(-2.668, 5.319)
	actual := v1.Angle(v2)
	...
	if actual != expect {
		t.Logf("Vector angle failed, expecting %v, got %v", expect, actual)
		t.Fail()
	}
